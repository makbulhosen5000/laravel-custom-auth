    ...
    <div class="card flex-shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
      <form action={{ route('login-user') }} method="post" class="card-body">
        ...
        <div class="form-control">
          ...
          <input type="password" name="password" value="{{ old('email') }}" placeholder="password" class="input input-bordered" />
          <span class="text-red-600">@error('password') {{ $message }} @enderror</span>
          <label class="label">
            <a href="#" name="password" class="label-text-alt link link-hover">Forgot password?</a>
          </label>
        </div>
